...
public class GetDateList {
    ...
    public ArrayList<Long> getMonth(long inputMillis) {
        ...
        Log.e("TAG", "当前时间 ： =======" + MyTimeUtils.millis2String(inputMillis, TimeUtils.DATE_FORMAT_DATE));
        //根据年份来返回对应的月份数据
    ...
    public ArrayList<Long> getDay(long inputMillis) {
        ...
        Log.e("TAG", "===================================== :      " + MyTimeUtils.date2String(date, MyTimeUtils.DATE_FORMAT_DATE));
...
//            }
     ...
     * @param month
     */
    public long isChangeDate(long time, int month) {
        int day = 0;
        int inputMonth = 0;
        Date date = MyTimeUtils.millis2Date(time);
        if (isAdd) {
        ...
        if (inputMonth != month) {
            if (month == 2) {
                if (MyTimeUtils.isLeapYear(date.getYear()))
                    day = 29;
                else
                    day = 28;
            ...
            if (isAdd) {
                date.setMonth(inputMonth);//需要传入原始数据
            } else {
                date.setMonth(month - 1);//起始值为0因此-1
            }
            date.setDate(day);
            time = MyTimeUtils.date2Millis(date);
        }
        return time;
    }
